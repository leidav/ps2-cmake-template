...
VECTOR light_direction[4] = {{0.00f, 0.00f, 0.00f, 1.00f},
                             {1.00f, 0.00f, -1.00f, 1.00f},
                             {0.00f, 1.00f, -1.00f, 1.00f},
                             {-1.00f, -1.00f, -1.00f, 1.00f}};

VECTOR light_colour[4] = {{0.00f, 0.00f, 0.00f, 1.00f},
                          {1.00f, 0.00f, 0.00f, 1.00f},
                          {0.30f, 0.30f, 0.30f, 1.00f},
                          {0.50f, 0.50f, 0.50f, 1.00f}};

int light_type[4] = {LIGHT_AMBIENT, LIGHT_DIRECTIONAL, LIGHT_DIRECTIONAL,
                     LIGHT_DIRECTIONAL};
...
void flip_buffers(packet_t *flip, framebuffer_t *frame)
{
	...
	dma_wait_fast();
	dma_channel_send_normal_ucab(DMA_CHANNEL_GIF, flip->data, q - flip->data,
	                             0);
...
int render(framebuffer_t *frame, zbuffer_t *z)
{
	...
	for (;;) {
		...
		q = render_teapot(q, view_screen, object_position, object_rotation,
		                  &prim, &color, frame, z);
		...
		draw_wait_finish();
		graph_set_framebuffer_filtered(frame[context].address,
		                               frame[context].width, frame[context].psm,
		                               0, 0);
